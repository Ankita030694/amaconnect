...
export default function FailureClient() {
  ...
  useEffect(() => {
    ...
    // Update checkout log status in backend to 'failed'
    const registerFailure = async () => {
      try {
        ...
        console.error("Failed to register failure callback status", err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#F5F2EB] text-gray-800 pt-[52px] font-sans">
      ...
      <main className="flex-grow py-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden flex items-center justify-center">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-[radial-gradient(ellipse_at_center,rgba(220,38,38,0.06),transparent_60%)] rounded-full blur-3xl -z-10 pointer-events-none" />
          ...
          <div className="bg-white border border-[#D4AF37]/35 shadow-2xl rounded-3xl p-8 text-center flex flex-col items-center transform transition-all hover:scale-[1.01]">
            ...
            {isLoading ? (
              ...
              <div className="animate-in fade-in zoom-in duration-300 flex flex-col items-center">
                ...
                <p className="text-gray-500 text-sm leading-relaxed mb-8 max-w-sm">
                  Unfortunately, your payment attempt could not be processed successfully. No funds have been captured, and your checkout log has been updated to failed status.
                </p>
