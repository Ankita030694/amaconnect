...
/** Shared layout tokens — slide 1 & 2 use identical outer dimensions */
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    }, 5000); // 5 seconds for hero slides to give user time to absorb content
  ...
  return (
    ...
    >
      {/* Slide 3 Edge-to-Edge Full-Bleed Background Graphic (Covers full hero section) */}
      <div 
        className={`absolute inset-0 z-0 select-none pointer-events-none overflow-hidden transition-opacity duration-700 ease-in-out hidden lg:block ${
      ...
      <div className="max-w-7xl mx-auto w-full relative z-20 h-full flex items-center">
        ...
        {/* SLIDE 1: Community */}
        <div
          className={`w-full h-full overflow-visible transition-all duration-700 ease-in-out ${currentSlide === 0
        ...
        {/* SLIDE 2: Interviews heading + Top Lawyer Stories */}
        <div
        ...
        {/* SLIDE 3: AMA Connect Legal Aid App Showcase */}
        <div
          className={`w-full h-full transition-all duration-700 ease-in-out ${currentSlide === 2
            ? "opacity-100 translate-x-0 relative pointer-events-auto scale-100 z-10"
            : "opacity-0 absolute inset-0 pointer-events-none scale-[0.98] -translate-x-4 z-0"
            }`}
        >
      ...
      {/* Dynamic Navigation Indicator Dots (Positioned relative to the section base) */}
