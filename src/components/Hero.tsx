...
// Unused community orbit and layout tokens commented out:
//
...
const cleanNewsDescription = (text: string) => {
  if (!text) return "";
  return text.replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim();
};

const truncateNewsWords = (text: string, maxWords: number) => {
  if (!text) return "";
...
function HeroNews() {
  ...
  useEffect(() => {
    async function fetchInitialNews() {
      try {
        ...
        console.error("Error loading news for Hero:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  const loadMoreArticles = async () => {
    ...
    try {
      ...
      console.error("Error loading more articles in Hero:", err);
    } finally {
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    // Detect if we scrolled within 300px of the right end of the horizontal container
  ...
  // Pastels/Warm gradients matching CourtroomExperiences styles
  const gradients = [
...
export default function Hero({ initialInterviews = [] }: { initialInterviews?: LawyerInterview[] } = {}) {
  ...
  return (
    ...
    >
      {/* Background Graphic for the App Slide (Commented out)
      <div
      ...
      */}
      ...
      <div className="max-w-7xl mx-auto w-full relative z-20 h-full flex items-center">
        ...
        {/* SLIDE 1 (Index 0): Interviews heading + Top Lawyer Stories */}
        <div
        ...
        {/* SLIDE 2 (Index 1): Indian Court News & Updates */}
        <div
        ...
        {/* Commented Out App Showcase and WhatsApp Community Slides
        // OLD SLIDE 2 (Index 1): AMA Connect Legal Aid App Showcase
        <div
          className={`w-full h-full transition-all duration-700 ease-in-out ${currentSlide === 1
            ? "opacity-100 translate-x-0 relative pointer-events-auto scale-100 z-10"
            : "opacity-0 absolute inset-0 pointer-events-none scale-[0.98] -translate-x-4 z-0"
