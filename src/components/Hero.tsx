...
export default function Hero({ initialInterviews = [] }: { initialInterviews?: LawyerInterview[] } = {}) {
  ...
  return (
    <section 
      ...
      className="relative w-full overflow-hidden pt-12 sm:pt-16 pb-12 sm:pb-16 px-4 sm:px-6 lg:px-8 bg-white flex flex-col items-center justify-center min-h-[600px] font-sans"
    >
      <div className="max-w-7xl mx-auto w-full relative z-20 flex flex-col gap-6">
        
        {/* Centered Catchy Big Heading */}
        <div className="text-center mb-6 sm:mb-10">
          <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black text-[#2D2219] tracking-tight leading-none">
            Top <span className="text-[#D4AF37]">Lawyer Stories</span>
          </h2>
          <p className="text-gray-500 font-medium text-xs sm:text-sm lg:text-base mt-4 max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full items-stretch">
          ...
          {featuredStory && (
            <div className="lg:col-span-7 bg-white border border-gray-150 rounded-3xl overflow-hidden transition-all duration-300 flex flex-col h-full hover:shadow-md group">
              {/* Image Container with custom background color if available */}
              <Link 
                href={`/interviews/${featuredStory.slug}`} 
                className={`relative block overflow-hidden rounded-t-3xl aspect-[1478/831] flex items-center justify-center ${featuredStory.bgColor || "bg-[#ECE8F5]"}`}
              >
                <img
                ...
                <div className="absolute inset-0 bg-black/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <div className="bg-[#D4AF37] text-white p-3.5 rounded-full shadow-lg transform scale-90 group-hover:scale-100 transition-transform duration-300">
              ...
              <div className="p-6 flex flex-col justify-between flex-grow">
                <div>
                  ...
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-[11px] sm:text-xs font-bold text-[#C79116] uppercase tracking-wider">
                  ...
                  <Link href={`/interviews/${featuredStory.slug}`}>
                    <h4 className="font-extrabold text-[#2D2219] group-hover:text-[#D4AF37] transition-colors leading-tight text-base sm:text-lg lg:text-xl line-clamp-1 mb-2">
                      {featuredStory.title}
                    </h4>
                  </Link>

                  {/* Description */}
                  <p className="text-xs sm:text-sm text-gray-500 font-medium leading-relaxed line-clamp-2 mb-4">
                    {truncateWords(cleanDescription(featuredStory.description), 35)}
                  </p>
                </div>

                {/* Read Full / Share Row */}
                <div className="flex justify-between items-center pt-3.5 border-t border-dashed border-gray-150 mt-auto">
                  <Link href={`/interviews/${featuredStory.slug}`} className="text-xs sm:text-sm font-extrabold text-[#D4AF37] hover:text-[#B8860B] transition-colors flex items-center gap-1">
                    Read Full Interview
                  </Link>
                  <button className="text-gray-400 hover:text-[#D4AF37] transition-colors p-1.5 rounded-full hover:bg-gray-50 cursor-pointer">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4 justify-start">
            {sidebarStories.map((story) => (
              <div 
                key={story._id} 
                className="bg-white border border-gray-100 hover:border-gray-250 rounded-3xl p-4 flex gap-4 transition-all duration-300 hover:shadow-sm h-[120px] sm:h-[135px] items-stretch group overflow-hidden"
              >
                {/* Left Details */}
                <div className="flex-grow flex flex-col justify-between py-0.5 flex-1 min-w-0">
                  <div>
                    ...
                    <Link href={`/interviews/${story.slug}`}>
                      <h5 className="font-extrabold text-[#2D2219] group-hover:text-[#D4AF37] transition-colors leading-snug text-xs sm:text-sm line-clamp-2 mb-1">
                        {story.title}
                      </h5>
                    </Link>
                  </div>
                  {/* Read More */}
                  <Link href={`/interviews/${story.slug}`} className="text-xs font-bold text-[#D4AF37] hover:text-[#B8860B] transition-colors mt-auto block">
                    Read More
                  </Link>
                </div>
                ...
                <Link 
                  ...
                  className="self-center h-full aspect-[1478/831] rounded-2xl overflow-hidden shrink-0 bg-[#EFEFEF] relative flex items-center justify-center border border-gray-50"
                >
                  <img
            ...
            {[...Array(totalSlides)].map((_, idx) => (
              <button
                ...
                className={`h-2.5 rounded-full transition-all duration-300 cursor-pointer ${
                  currentSlide === idx ? "w-8 bg-[#2D2219]" : "w-2.5 bg-[#2D2219]/25 hover:bg-[#2D2219]/55"
                }`}
